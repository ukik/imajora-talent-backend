    ...
    function FOURTH($faker) {
        ...
        \UserImage::query()
            ->chunk(100, function($datas) use ($faker, $images) {
                ...
                // \UserImageComments::query()->insert($temp_comments);
                \UserImageFile::query()
    ...
    function THIRD($faker) {
        \UserImageComments::query()
            ...
            ->chunk(100, function($datas) use ($faker) {
                ...
                // \UserImageComments::query()->insert($temp_comments);
                \UserImageComments::query()->upsert($temp_comments, ['post_id','user_id','parent_id','comment']);
            });
    }
    ...
    function SECOND($faker) {
        ...
        \UserImage::query()
            ->chunk(100, function($datas) use ($faker) {
                ...
                // \UserImage::query()->upsert($temp_video, ['description']);
                // \UserImageComments::query()->insert($temp_comments);
                \UserImageComments::query()->upsert($temp, ['post_id','user_id','parent_id','comment']);
            });
    ...
    function FIRST($faker) {
        ...
        \UserImage::query()
            ->chunk(100, function($datas) use ($faker) {
                ...
                \UserImage::query()->upsert($temp, ['description']);
